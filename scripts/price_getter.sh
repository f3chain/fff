...
if [ $# -le 0 ]; then echo "Usage: $0 http://ip:port account_name _FFFmarketcat_api_key"; exit ; fi
endpoint=$1
account=$2
CMC_API_KEY=$3
max_length=17
...
do
   price=`curl -s -H "X-CMC_PRO_API_KEY: $CMC_API_KEY" "https://pro-api._FFFmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=DCT&convert=$c" | grep -i price | grep -o '[0-9]*\.\?[0-9]\+'`
