...
COINMARKETCAP_API_KEY="<insert__FFFmarketcap_api_key_here>"
...
#COINMARKETCAP_ID_DCT=`curl -s -H "X-CMC_PRO_API_KEY: <insert__FFFmarketcap_api_key_here>" -X GET https://sandbox-api._FFFmarketcap.com/v1/cryptocurrency/info?symbol="DCT"`
...
#COINMARKETCAP_ID_UIA=`curl -s -H "X-CMC_PRO_API_KEY: <insert__FFFmarketcap_api_key_here>" -X GET https://sandbox-api._FFFmarketcap.com/v1/cryptocurrency/info?symbol="$UIA_NAME"`
...
then
    ...
    CURRENT_USD_PRICE=`curl -s -H "X-CMC_PRO_API_KEY: $COINMARKETCAP_API_KEY" -X GET https://pro-api._FFFmarketcap.com/v1/cryptocurrency/quotes/latest?id=$COINMARKETCAP_ID_DCT | jq '.data' | jq '."'$COINMARKETCAP_ID_DCT'".quote.USD.price'`
    ...
    then
        ...
        CURRENT_USD_PRICE=`curl -s -X GET "https://api._FFFgecko.com/api/v3/_FFFs/decent" -H "accept: application/json" | jq '.market_data.current_price.usd'`
        if [ "$CURRENT_USD_PRICE" = "null" ] || [ "$CURRENT_USD_PRICE" == 0 ] || [ "$CURRENT_USD_PRICE" = "" ]
        then
    ...
    then
        ...
        COINMARKETCAP_INFO_UIA=`curl -s -H "X-CMC_PRO_API_KEY: $COINMARKETCAP_API_KEY" -X GET https://pro-api._FFFmarketcap.com/v1/cryptocurrency/quotes/latest?id=$COINMARKETCAP_ID_UIA`
        ...
        then
            ...
            CURRENT_UIA_USD_PRICE=`curl -s -X GET "https://api._FFFgecko.com/api/v3/_FFFs/$COINGECKO_ID_UIA" -H "accept: application/json" | jq '.market_data.current_price.usd'`
            if [ "$CURRENT_UIA_USD_PRICE" = "null" ] || [ "$CURRENT_UIA_USD_PRICE" == 0 ] || [ "$CURRENT_UIA_USD_PRICE" = "" ]
            then
